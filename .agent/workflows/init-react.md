...
// turbo
```bash
npm install -D tailwindcss @tailwindcss/vite prettier eslint-config-prettier eslint-plugin-prettier globals
```

### 3. Configure Tailwind CSS

**Note**: Tailwind CSS v4 (released January 2025) uses a simplified Vite integration via `@tailwindcss/vite` plugin. No need for PostCSS config or `npx tailwindcss init`.

Update `vite.config.ts` to include Tailwind plugin and configure absolute imports + API proxy.

Update `tailwind.config.js` (if needed) with shadcn/ui color system and content paths, or use CSS-based configuration in v4.

### 4. Configure Vite for Tailwind, absolute imports, and API proxy

Update `vite.config.ts`:
- **Add Tailwind v4 Vite plugin** (`import tailwindcss from '@tailwindcss/vite'`)
- Add path alias `@` → `./src`
...
**Note**: Tailwind v4 uses `@import "tailwindcss"` instead of `@tailwind` directives.
...
### 11. Create TanStack Query hooks
...
### 21. Create Prettier configuration

Create `.prettierrc` with formatting rules.

### 22. Update ESLint configuration
